...
public class PaymentOrderActivity extends BaseActivity {
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        mAmount = getIntent().getIntExtra("Amount", -1);//商品数量
    ...
    @Override
    @OnClick({R.id.iv_back, R.id.iv_down, R.id.iv_top, R.id.cb_aliPay, R.id.tv_pay, R.id.Ll_confirmOrder})
    ...
    private void getOrder() {
        ...
        params.put("receiptAddress", tv_address.getText().toString());//收货地址
        params.put("cusId", SpfUtils.getSpfSaveInt(Constant.userId));
        params.put("orderPay", mPriceNow);//订单总价
        params.put("goodsPay", mPriceNow);//商品总价
        params.put("logisticsPay", 0);//运费
        params.put("goodsId", mId);//商品id
        params.put("goodsCount", 1);//购买数量
        params.put("goodsPrice", mPriceNow);//商品成交价
